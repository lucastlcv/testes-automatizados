...
context('', () => {
    ...
    it('Teste Faturamento', () => {
        ...
        cy.get('.text-center > .btn').click({force:true})
        cy.get('tbody.ng-star-inserted').find('tr').should('have.length', 3)
        cy.get('.modal-footer > .btn').click({force:true})
        cy.wait(1000)
        /*cy.get(':nth-child(1) > :nth-child(1) > .text-filters > .nome-filtro').click({force:true})
        cy.get('#searchDropdown0').click({force:true})
            .type('Milvus inovacoes em software')
            .wait(500)
        cy.get('#opt-0-105798 > .nome-filtro').click({force:true})
        cy.get('.filtro-controls > :nth-child(2) > .btn').click({force:true})*/
